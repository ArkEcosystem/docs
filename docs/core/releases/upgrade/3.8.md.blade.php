...
1. Stop Core and install latest PostgreSQL.
...
2. Drop the default new PostgreSQL cluster created with the install and perform the upgrade process.
...
3. Make sure you are able to list DBs.
...
4. Remove the old PostgreSQL version and PG cluster.

```bash
sudo apt-get purge postgresql-contrib postgresql-$(pg_lsclusters -h | awk 'NR==1' | awk '{print $1}') -y
```

5. Finally restart your new PostgreSQL and start Core.
