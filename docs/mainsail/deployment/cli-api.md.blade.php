...
- Run `mainsail-api api:status --token=ark --network=testnet` to ensure that the core to check the status of your process.

You can view your `api` process logs with the following command:

```shell
mainsail-api api:log --token=ark --network=testnet
```

- Check the logs to ensure that the process has started correctly

<x-alert type="info">
If your `api` process isn't running, then an error should be displayed. You can check the status of your `api` process with `mainsail-api core:status --token=ark --network=testnet`
</x-alert>

You can restart your `api` process with the following command:

```shell
mainsail-api api:restart --token=ark --network=testnet
```
...
```shell
mainsail-api api:stop --token=ark --network=testnet
```

You can run the `api` process in foreground with the following command

```shell
mainsail-api api:run --token=ark --network=testnet
```

- Exit the process and ensure that it has stopped from running with `mainsail-api api:stop --token=ark --network=testnet`