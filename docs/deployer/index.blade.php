<x-general.sidebar-link first top-level path="/docs/deployer/intro" name="Introduction" />

<x-general.sidebar-group path="/docs/deployer/prepare/*" title="Prepare">
    <x-general.sidebar-link path="/docs/deployer/prepare/prepare-introduction" name="Prepare Introduction" />
    <x-general.sidebar-link path="/docs/deployer/prepare/designing-blockchain-parameters" name="Designing Blockchain Parameters" />
    <x-general.sidebar-link path="/docs/deployer/prepare/network-requirements" name="Network Requirements" />
    <x-general.sidebar-link path="/docs/deployer/prepare/vps-dedicated-local" name="VPS vs Dedicated vs Local Server" />
    <x-general.sidebar-link path="/docs/deployer/prepare/creating-hardware-obtaining-ip-addresses" name="Creating Hardware and Obtaining IP Addresses" />
    <x-general.sidebar-link path="/docs/deployer/prepare/connect-to-servers" name="Connect To Servers" />
    <x-general.sidebar-link path="/docs/deployer/prepare/preparing-server" name="Preparing Your Server" />
    <x-general.sidebar-link path="/docs/deployer/prepare/preparing-github" name="Preparing Github" />
</x-general.sidebar-group>

<x-general.sidebar-group path="/docs/deployer/customize/*" title="Customize">
    <x-general.sidebar-link path="/docs/deployer/customize/customize-introduction" name="Customize Introduction" />
    <x-general.sidebar-link path="/docs/deployer/customize/customization-levels" name="Customization Levels" />
    <x-general.sidebar-link path="/docs/deployer/customize/name-your-bridgechain" name="Name Your Bridgechain" />
    <x-general.sidebar-link path="/docs/deployer/customize/configure-bridgechain-parameters" name="Configure Bridgechain Parameters" />
    <x-general.sidebar-link path="/docs/deployer/customize/static-fees" name="Static Fees" />
    <x-general.sidebar-link path="/docs/deployer/customize/dynamic-fees" name="Dynamic Fees" />
    <x-general.sidebar-link path="/docs/deployer/customize/configure-bridgechain-installation" name="Configure Bridgechain Installation" />
    <x-general.sidebar-link path="/docs/deployer/customize/explorer" name="Explorer" />
</x-general.sidebar-group>

<x-general.sidebar-group path="/docs/deployer/deploy/*" title="Deploy">
    <x-general.sidebar-link path="/docs/deployer/deploy/deploy-introduction" name="Deploy Introduction" />
    <x-general.sidebar-link path="/docs/deployer/deploy/transferring-and-executing-script" name="Transferring and Executing Installation Script" />
    <x-general.sidebar-link path="/docs/deployer/deploy/running-and-managing-core-processes" name="Running and Managing Core Processes" />
    <x-general.sidebar-link path="/docs/deployer/deploy/committing-core-changes-to-github-repo" name="Committing Core Changes to GitHub Repo" />
    <x-general.sidebar-link path="/docs/deployer/deploy/adding-network-peers" name="Adding Network Peers" />
    <x-general.sidebar-link path="/docs/deployer/deploy/adding-bridgechain-to-ark-desktop-wallet" name="Adding Bridgechain to ARK Desktop Wallet" />
    <x-general.sidebar-link path="/docs/deployer/deploy/importing-genesis-addresses-and-adding-forgers" name="Importing Genesis Addresses and Adding Forgers" />
    <x-general.sidebar-link path="/docs/deployer/deploy/updating-your-bridgechain" name="Updating Your Bridgechain" />
    <x-general.sidebar-link path="/docs/deployer/deploy/security-notice" name="Security Notice" />
    <x-general.sidebar-link path="/docs/deployer/deploy/whats-next" name="Whats Next" />
</x-general.sidebar-group>
