...
You will also need to install the [Desktop Wallet](https://ark.io/wallet) if you haven’t already.
...
`"description": "Desktop Wallet Theme Template",` — a short description of the plugin, in our case this can be just something generic like what color tone you are using eg. `"description": "`_**`Pinkish ARK Desktop Wallet theme`**_`",`
...
The last part of this guide is now to publish a plugin on the NPM. To do so we will need NodeJS installed (you can download it from **[https://nodejs.org](https://nodejs.org)** for your OS ("recommended for most users" version). Once installed open command line/terminal.
...
`git clone https://github.com/boldninja/theme-template.git`
...
Now it will take some time to publish it to the NPM and if there are no errors then it should be complete. Your template will be published on NPM and should be visible by going to `https://www.npmjs.com/package/<@your-username>/<your-package-name>`.
