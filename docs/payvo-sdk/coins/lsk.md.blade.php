---
title: LSK
---

# Lisk

This is a Lisk Driver for the Platform SDK. The implementation makes use of [Lisk](https://lisk.io/) and adheres to the contracts laid out in the [specification](/docs/payvo-sdk/specification). **This package should not be used directly. It is designed to be consumed through the [SDK](/docs/payvo-sdk/sdk) package.**

## Repository

<livewire:embed-link url="https://github.com/PayvoHQ/sdk/tree/master/packages/sdk-lsk" />

## Installation

```bash
yarn add @payvo/sdk-lsk
```

## Specification

<livewire:coin-spec spec="payvo-sdk/coins/specs/lsk.json" />

## Security

If you discover a security vulnerability within this package, please send an e-mail to [security@ark.io](mailto:security@ark.io). All security vulnerabilities will be promptly addressed.
