...
The documentation within [Core](/docs/core/installation/intro) is an ideal way for developers to go through the processes for setting up a development environment. We recommend Linux (_\*.deb based_) operating system as the default environment. We officially recommend and support **Ubuntu** operating system.

You can also set up your development environment using [Docker on Unix Linux/Macos](/docs/core/installation/docker-unix) or a [Docker on Windows](/docs/core/installation/docker-windows).
...
After creating and logging under `ark` user, you can execute the following script to install development tools and dependencies from [Technology Stack](/docs/core/overview/stack). If your default user isn't `ark`, you should copy and modify the script source below \(database section\).
